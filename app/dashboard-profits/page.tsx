...
import { Home as HomeIcon, TrendingUp, Percent, MapPin } from "lucide-react";
  ...
  return (
    <div className={cn("flex flex-col items-center", className)}>
      <svg viewBox="0 0 200 60" className="w-full h-20">
        <polygon points="0,50 20,40 40,45 60,30 80,35 100,20 120,25 140,10 160,20 180,10 200,30 200,60 0,60" fill="#6366f1" opacity="0.15" />
        <polyline points="0,50 20,40 40,45 60,30 80,35 100,20 120,25 140,10 160,20 180,10 200,30" fill="none" stroke="#6366f1" strokeWidth="2" />
      </svg>
      ...
      <div className="flex w-full items-end justify-between h-24">
        {bars.map((h, i) => (
          <div key={i} className="flex flex-col items-center w-6">
            <div style={{ height: `${h}px` }} className="w-4 bg-indigo-400 rounded-t" />
          </div>
        ))}
      </div>
...
// sample data lang
const topProducts = [
  { name: "Product Alpha", profit: "$4,200" },
  { name: "Product Beta", profit: "$3,150" },
  { name: "Product Gamma", profit: "$2,980" },
];
const recentTransactions = [
  { id: "TXN-001", date: "2024-06-01", amount: "+$1,200", desc: "Wholesale Profit" },
  { id: "TXN-002", date: "2024-06-02", amount: "+$980", desc: "Retail Profit" },
  { id: "TXN-003", date: "2024-06-03", amount: "+$2,100", desc: "Online Sales" },
  { id: "TXN-004", date: "2024-06-04", amount: "+$1,500", desc: "Bulk Order" },
];
const monthsArr = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
const barHeights = [30, 45, 20, 60, 50, 80, 40, 70, 55, 65, 35, 90];
...
export default function DashboardProfitsPage() {
  return (
    <div className="min-h-screen bg-white text-gray-900 flex flex-col">
      ...
      <div className="max-w-7xl mx-auto w-full px-2 sm:px-4 mt-6 mb-4">
        <Card className="bg-white border-gray-200 p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-4 shadow-sm">
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <Label className="flex items-center gap-1 text-gray-600 text-xs">
              <Input type="checkbox" className="mr-1 h-3 w-3" defaultChecked /> Profitable only
            </Label>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 mt-8">
          ...
          <Card className="bg-white text-gray-900 border-gray-200 flex flex-col justify-between min-h-[220px] col-span-1 shadow-sm">
            <CardHeader className="flex flex-row items-center gap-3 pb-2">
              ...
              <div>
                <CardDescription className="text-gray-500">Total Profits</CardDescription>
                <CardTitle className="text-3xl">$32,180</CardTitle>
              </div>
            </CardHeader>
            <CardContent>
              <span className="text-green-600 text-base font-semibold">+12.5% this month</span>
              <AreaChartPlaceholder className="mt-2" label="Total Profits" />
            </CardContent>
          </Card>
          {/* Profit Margin */}
          <Card className="bg-white text-gray-900 border-gray-200 flex flex-col justify-between min-h-[220px] col-span-1 shadow-sm">
            <CardHeader className="flex flex-row items-center gap-3 pb-2">
              ...
              <div>
                <CardDescription className="text-gray-500">Profit Margin</CardDescription>
                <CardTitle className="text-3xl">38.2%</CardTitle>
              </div>
            </CardHeader>
            <CardContent>
              <span className="text-blue-600 text-base font-semibold">+2.1% from last month</span>
              <AreaChartPlaceholder className="mt-2" label="Profit Margin" />
            </CardContent>
          </Card>
          {/* Profit Trend Chart */}
          <Card className="bg-white text-gray-900 border-gray-200 min-h-[260px] col-span-1 md:col-span-2 xl:col-span-1 shadow-sm">
            <CardHeader>
              <CardTitle className="text-2xl">Profit Trend</CardTitle>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
          ...
          <Card className="bg-white text-gray-900 border-gray-200 min-h-[280px] md:col-span-2 shadow-sm">
            <CardHeader className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                <CardTitle className="text-2xl">Profits by Month</CardTitle>
                <CardDescription className="text-gray-500">2024</CardDescription>
              </div>
            </CardHeader>
            <CardContent>
              <BarChartWithLabels bars={barHeights} months={monthsArr} label="Monthly Profits" />
            </CardContent>
          </Card>
          {/* Top Products */}
          <Card className="bg-white text-gray-900 border-gray-200 min-h-[220px] shadow-sm">
            <CardHeader>
              <CardTitle className="text-2xl">Top Profitable Products</CardTitle>
              ...
              <ul className="space-y-4">
                {topProducts.map((p, i) => (
                  <li key={i} className="flex justify-between items-center">
                    ...
                    <span className="text-green-600 font-semibold text-lg">{p.profit}</span>
                  </li>
                ))}
              </ul>
              {/* SUKATAN: data */}
          ...
          <Card className="bg-white text-gray-900 border-gray-200 min-h-[220px] shadow-sm">
            <CardHeader>
              <CardTitle className="text-2xl">Recent Profit Transactions</CardTitle>
                  ...
                  <tbody>
                    {recentTransactions.map((txn, i) => (
                      <tr key={i} className="border-b border-gray-200 last:border-0">
                        ...
                        <td>{txn.date}</td>
                        <td>{txn.desc}</td>
                        <td className="text-green-600 text-right">{txn.amount}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <Footer year="2025" companyName="Company Name" />
    </div>
  );
} 